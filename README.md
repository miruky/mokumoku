...
- ログの上には最近7日の集中本数がミニ棒グラフで並びます。目標に届いた日が連続している間は「連続達成 N日」が表示され、点線が目標ラインです
- 「日報を生成」で今日のログがMarkdownになります。末尾に直近7日の集中本数と連続達成日数が添えられ、コピーまたは `nippou-YYYY-MM-DD.md` としてダウンロードできます
- ログの各行にマウスを重ねるかフォーカスすると削除ボタンが出ます。タグを間違えて記録したセッションをその場で消せます
- 「時間と目標の設定」で集中・休憩の長さ、長休憩の間隔に加えて、1日に積みたい集中の目標本数を変えられます。目標はログの上のメーターに反映されます。進行中のフェーズは打ち切らず、次のフェーズから反映されます

マウスを使わずに操作できるよう、主要な操作にショートカットを割り当てています。`Space` で開始と一時停止、`S` でスキップ、`R` でリセット、`G` で日報。作業名の入力中や日報プレビューを開いている間は無効になります。
...
- `src/lib/sessions.ts` — 集中セッションの台帳。検証・直列化・作業別集計・削除
- `src/lib/report.ts` — 1日分のセッションから日報Markdownを生成
- `src/lib/history.ts` — 直近の日別集計と目標達成の連続日数の算出
- `src/lib/goal.ts` — 1日の目標本数に対する達成率の算出
- `src/lib/shortcuts.ts` — 押されたキーから操作を引くショートカットの解決
