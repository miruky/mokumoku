...
function loadConfig(storage: Storage): TimerConfig {
  try {
    ...
    if (raw !== null) {
      ...
      if (isValidConfig(parsed as TimerConfig)) return parsed as TimerConfig;
    }
  } catch {
    // 壊れた保存値は既定値に戻す
  }
...
const APP_HTML = `
  ...
  <main>
    <section class="card timer-card" aria-label="ポモドーロタイマー">
      <div class="phase-row">
        <span class="phase-badge" id="phase-badge">集中</span>
    ...
    <section class="card log-card" aria-label="今日の作業ログ">
      <div class="log-head">
        <h2>今日のログ</h2>
...
export function mountApp(
  ...
  const phaseBadge = el<HTMLSpanElement>(root, '#phase-badge');
  ...
  function renderLog(): void {
    ...
    listEl.innerHTML = today
      .map((s, i) => {
        ...
        return (
          `<li style="--stagger:${i}">` +
          `<span class="session-range">${formatClock(s.startedAt)}-${formatClock(s.endedAt)}</span>` +
          `<span class="session-task">${escapeHtml(s.task)}</span>${mark}</li>`
        );
      })
      .join('');
  }
